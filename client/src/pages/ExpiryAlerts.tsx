...
export default function ExpiryAlerts() {
  ...
  const getStatusBadge = (
    ...
    switch (status) {
      ...
      case "expiring-soon":
        return (
          <Badge variant="destructive">Expiring in {daysToExpiry} days</Badge>
        );
      case "expiring-this-month":
        return (
          <Badge variant="secondary">Expiring in {daysToExpiry} days</Badge>
        );
      default:
        return null;
    }
  ...
  const expiringSoonProducts = expiryProducts.filter(
    (p) => p.status === "expiring-soon",
  );
  const expiringThisMonthProducts = expiryProducts.filter(
    (p) => p.status === "expiring-this-month",
  );

  const totalValueAtRisk = expiryProducts.reduce(
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">{t("expiry.subtitle")}</p>
      </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-yellow-600">
              {expiringThisMonthProducts.length}
            </div>
            <p className="text-xs text-muted-foreground">
              {t("expiry.within30Days")}
            </p>
          </CardContent>
        </Card>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              ...
              <SelectContent>
                ...
                <SelectItem value="expired">{t("expiry.expired")}</SelectItem>
                <SelectItem value="expiring-soon">
                  {t("expiry.expiringSoon")}
                </SelectItem>
                <SelectItem value="expiring-this-month">
                  {t("expiry.thisMonth")}
                </SelectItem>
              </SelectContent>
            </Select>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("expiry.productExpiryStatus")}</CardTitle>
          <CardDescription>
            {filteredProducts.length} of {expiryProducts.length} products with
            expiry concerns
          </CardDescription>
        </CardHeader>
        <CardContent>
          {filteredProducts.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              {searchTerm || statusFilter !== "all"
                ? t("expiry.noProductsFilter")
                : t("expiry.noProductsExpiry")}
            </div>
          ) : (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>{t("expiry.valueAtRisk")}</TableHead>
              ...
              {expiredProducts.length > 0 && (
                <div className="p-4 border border-red-200 rounded-lg bg-red-50">
                  ...
                  <p className="text-red-700 text-sm mb-3">
                    {expiredProducts.length} products have expired and should be
              ...
              {expiringSoonProducts.length > 0 && (
                <div className="p-4 border border-orange-200 rounded-lg bg-orange-50">
                  ...
                  <p className="text-orange-700 text-sm mb-3">
                    {expiringSoonProducts.length} products expire within 7 days.
              ...
              {expiringThisMonthProducts.length > 0 && (
                <div className="p-4 border border-yellow-200 rounded-lg bg-yellow-50">
                  <h4 className="font-semibold text-yellow-800 mb-2">
                    Medium Priority - Expiring This Month
                  </h4>
                  <p className="text-yellow-700 text-sm mb-3">
                    {expiringThisMonthProducts.length} products expire within 30
                    days. Plan ahead.
                  </p>
